...
describe('Game View Component', () => {
    ...
    it('renders `CANCELED` game successfully', () => {
        ...
        customRender(<MockRouter query={{ game: CANCELED_GAME.id }}>
            <GameView />
        </MockRouter>)
    ...
    it('renders `ONGOING` game successfully', () => {
        ...
        customRender(<MockRouter query={{ game: ONGOING_GAME.id }}>
            <GameView />
        </MockRouter>)
        ...
        expect(screen.getByText(`${ONGOING_GAME.homeScore.current} - ${ONGOING_GAME.awayScore.current}`)).toBeVisible()
    ...
    it('renders loading state', () => {
        customRender(<MockRouter isReady={false}>
            <GameView />
        </MockRouter>)
    ...
    it('renders not found state', () => {
        customRender(<MockRouter query={{ game: 'not-found' }}>
            <GameView />
        </MockRouter>)

        expect(screen.getByLabelText(/Caution Icon/i)).toBeVisible()
        expect(screen.getByRole('heading', { name: /not found/i })).toBeVisible()
        expect(screen.getByText(/Sorry, we couldn’t find the game you’re looking for./i)).toBeVisible()
